...
impl Ifacialmocap {
    ...
    fn _ready(&mut self, _owner: &Node) {
        godot_print!("iFM Tracker ready!");
    }
    fn register_signals(builder: &ClassBuilder<Self>) {
        builder.signal(&RUN_FACE_TRACKER_TEXT).done();
        builder.signal(&STOP_FACE_TRACKER_TEXT).done();
    }
    #[export]
    fn _process(&mut self, _owner: &Node, _delta: f64) {
        if self.is_listening(_owner) {
            ...
            self.data_map =
                IfacialmocapData::from_str(std::str::from_utf8(&buf[..size]).unwrap()).as_dict();
